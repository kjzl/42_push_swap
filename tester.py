...
def calculate_combination(nb):
	...
	all_moves = []
	all_res = []
	limit = 3 if nb == 3 else 12 if nb == 5 else 6 if nb == 4 else math.inf
	...
	if limit == 3:
		...
		all_too_much = [("./push_swap " + item[1] + " | wc -l") for item in all_moves if item[0] > 6]
	elif limit == 12:
	...
	if all_too_much:
		...
		if limit == 3:
			...
			f.write("** TOO MUCH MOVES FOR THESE TESTS (MAX = 6) **\n\n")
		elif limit == 12:
